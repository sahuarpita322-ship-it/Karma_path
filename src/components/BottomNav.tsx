...
import { FaCompass, FaLandmark, FaMap, FaWallet, FaUserCircle, FaBookOpen } from 'react-icons/fa';

const items = [
  { to: '/dashboard', icon: <FaCompass />, label: 'Home' },
  { to: '/tasks', icon: <FaLandmark />, label: 'Tasks' },
  { to: '/archive', icon: <FaBookOpen />, label: 'Archive' },
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 border-t border-sand bg-white/95 p-2 shadow-inner md:hidden">
      <div className="flex justify-between">
        {items.map((item) => (
          <Link key={item.to} to={item.to} className={`flex w-full flex-col items-center text-[10px] sm:text-xs ${location.pathname === item.to ? 'text-forest' : 'text-earth'}`}>
            {item.icon}
            {item.label}
          </Link>
        ))}
